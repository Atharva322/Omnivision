...
final class PersonStoreTests: XCTestCase {
    ...
    func testPronunciationPathCanBeAttachedDetachedAndReloaded() async throws {
        let url = temporaryStoreURL()
        let store = try PersonStore(url: url)
        let saved = try await store.bind(name: "Adaobi")
        let path = "/application-support/pronunciations/adaobi.caf"

        _ = try await store.setNamePronunciationPath(path, for: saved.id)
        let reloaded = try PersonStore(url: url)
        let persisted = await reloaded.find(id: saved.id)
        XCTAssertEqual(persisted?.namePronunciationPath, path)

        let detached = try await reloaded.setNamePronunciationPath(nil, for: saved.id)
        XCTAssertNil(detached.namePronunciationPath)
    }

    func testBlankPronunciationPathIsRejected() async throws {
        let store = try PersonStore(url: temporaryStoreURL())
        let saved = try await store.bind(name: "Priya")

        do {
            _ = try await store.setNamePronunciationPath("   ", for: saved.id)
            XCTFail("expected invalidPronunciationPath")
        } catch let error as PersonStoreError {
            XCTAssertEqual(error, .invalidPronunciationPath)
        }
    }
