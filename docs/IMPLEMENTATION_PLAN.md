...
- [x] `Narrator` — `AVSpeechSynthesizer`, priority queue, `repeatLast()`
- [x] **`holdUntilSilence()`** — never speak while the other person is speaking. Non-negotiable.
- [x] Earcon set: distinct tones for captureOn / captureOff / saved / unknown / **disconnected**
- [x] Write every spoken string. One action per sentence. Hard length caps.
- [x] **Hedge grammar** — the difference between "This might be Priya" and "This is Priya" is the whole
      accuracy claim, and it lives in string constants that are trivially easy to "improve" at Hour 9.
      Own them.
- [x] Consent script + the audible recording tone
- [x] Name-pronunciation capture: 1 s clip during consent, replayed instead of TTS
- [x] Human time formatting — "three weeks ago", never a timestamp
- [ ] Screen-free testing from Hour 3; run the 20-trial suite at 9:30
- [x] 2-minute demo, problem statement, backup video script
