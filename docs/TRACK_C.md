...
Meta glasses connectivity, Bluetooth routing, camera capture, the judge-mirror UI, and conversation
summarisation remain outside this package. Track A core, face clustering, `PersonStore`, relationship
tiers, identity resolution, and Track D narration now live beside Track C under
`Sources/AccessLensTrackC`; see [`TRACK_A.md`](./TRACK_A.md) and [`TRACK_D.md`](./TRACK_D.md).
